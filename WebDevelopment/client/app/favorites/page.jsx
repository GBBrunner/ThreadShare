...
export default function FavoritesPage() {
  ...
  const fetchPosts = async (newOffset = 0) => {
    try {
      const token = localStorage.getItem("auth_token");
      const res = await fetch(`${SERVER_URL}/api/favorite-posts?limit=10&offset=${newOffset}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleFavorite = async (e, postId) => {
    ...
    try {
      ...
      if (isLiked) setPosts((prev) => prev.filter((p) => p.id !== postId));
    } catch {
      setLikedPostIds((prev) => {
        const next = new Set(prev);
    ...
    <ProtectedRoute isLoggedIn={!!signed_in_user}>
      ...
      <div className="p-6">
        ...
        {loading ? (
          ...
          <p className="text-gray-500 text-center py-12">No favorited posts yet.</p>
        ) : (
          <>
