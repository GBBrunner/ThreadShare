...
router.post('/new_post', authenticateToken, upload.array('images', 5), async (req, res) => {
    try {
        ...
        if (req.body.occasions) {
            occasions = Array.isArray(req.body.occasions)
                ? req.body.occasions
                : JSON.parse(req.body.occasions);
        }
        ...
        if (invalidOccasion) {
            return res.status(400).json({ message: `Invalid occasion: ${invalidOccasion}` });
        }

        // file.path is the secure Cloudinary URL returned by multer-storage-cloudinary
        const imageUrls = (req.files || []).map(f => f.path);

        const result = await pool.query(
            `INSERT INTO posts
                (user_id, title, description, brand, size, category, condition, color, occasions, images)
